...
Return ONLY the JSON array, no additional text.`
    ...
    const groceryItems: GroceryItem[] = await Promise.all(
      parsedItems.map(async (item, index) => {
        ...
        const matches: ProductMatch[] = (searchResults || []).map((product: any) => {
          const offers = product.offers || [];
          const sortedOffers = [...offers].sort((a: any, b: any) => a.price_cents - b.price_cents);
          ...
          return {
            ...
            cheapest_store: cheapestOffer ? (storeDisplayNames[cheapestOffer.store] || cheapestOffer.store) : 'Unknown',
        ...
        const confidence: 'high' | 'medium' | 'low' = 
          ...
          matches.length > 1 && hasExactMatch ? 'medium' : 'low';
    ...
    const needsClarificationCount = groceryItems.filter(item => item.needs_clarification).length;
