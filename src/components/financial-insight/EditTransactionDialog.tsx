...
export const EditTransactionDialog = ({
  ...
  const { getSuggestions } = useMerchantSearch();
  const suggestions = getSuggestions(merchantName);
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              {showSuggestions && suggestions.length > 0 && (
                <ul className="absolute z-50 top-full mt-1 w-full bg-popover border border-border rounded-md shadow-md overflow-hidden">
                  {suggestions.map((s) => (
                    <li
                      key={s.id}
                      className="px-3 py-2 text-sm cursor-pointer hover:bg-accent"
                      onMouseDown={() => {
                        setMerchantName(s.merchant_name);
                        setDisplayName(s.display_name || extractDisplayMerchantName(s.merchant_name));
                        setSelectedParentCategory(s.category_id);
                        setShowSuggestions(false);
                      }}
                    >
                      <span className="font-medium">{s.display_name || s.merchant_name}</span>
                      {s.display_name && (
                        <span className="ml-2 text-xs text-muted-foreground">{s.merchant_name}</span>
                      )}
                    </li>
                  ))}
                </ul>
